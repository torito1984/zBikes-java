...
public class StationWithBikesMapper implements ResultSetMapper<Station> {

    private Station station;

    //This method will be called once per retrieved row
    //It will not be called if there are not rows retrieved
    @Override
    public Station map(int index, ResultSet r, StatementContext ctx) throws SQLException {
        if (index == 0) {
            ...
            station = new Station(name, location, new HashSet<>(), null, null, null);
        }
